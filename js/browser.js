...
function updateLayoutSize() {
	...
	if (maxEntriesContainerHeight < parseInt(accordion.css("min-height")))
		maxEntriesContainerHeight = parseInt(accordion.css("min-height"));

	accordion.height(maxEntriesContainerHeight);
	...
	var maxCategoriesListContainerHeight = Math.max(maxEntriesContainerHeight - filterPanelTopHeight, parseInt(accordion.css("min-height")));
...
function displayEntryDetails(id) {
	...
	$("#entryDetailsType" + entry.id).html("Type: " + entry.type);

	if (entry.url)
		$("#entryDetailsDOI" + entry.id).html("DOI/URL: <a href=\'" + entry.url + "\' target=\'_blank\'>" + entry.url + "</a>");

	$("#entryDetailsBib" + entry.id).html("<a href=\'bibtex/" + entry.id + ".bib\' target=\'_blank\'><span class=\'glyphicon glyphicon-save\'></span> BibTeX</a>");

	$("#entryDetailsCategories" + entry.id).empty();
	//$("#entryDetailsCategories" + entry.id).html("Categories: ");

	$.each(entry.categories, function(i,d){
		var item = categoriesMap[d];

		//var element = $("<span role=\"text\" aria-label=\'" + item.descriptionPrefix + item.description + "\' class=\"category-entry category-entry-span\" data-tooltip=\"tooltip\" tabindex=0></span>");
		var element = $("<span class=\'category-entry category-entry-span \'" +
					"data-tooltip=\'tooltip\' tabindex=0></span>");
					//"data-tooltip=\'tooltip\' role=\'text\' aria-label=\'"+ item.description + "\' tabindex=0></span>");
...
function updateDisplayedCount(){
	...
	$("#displayedEntriesCount").text($("#accordion .panel").size());
...
function appendCategoryFilter(item, parent, currentContainer, currentStats){
	...
	if (item.type == "category") {
		...
		var element = $("<button type=\"button\" class=\"btn btn-default category-entry active uniform\""
...
function getTimeChartEntryDescription(entry){
	if (!entry.gap) {
		return entry.year + ": "
			+ entry.current + " resources displayed, "
			+ entry.total + " resources in total";
	} else {
		return null;
	}
}
			...
			var element = $("<div class=\'panel panel-default\' tabindex=0>" +
								       ...
								       d.title + " (" + d.year + ")</a></h4></div>" +
									     ...
									     "<div class=\'panel-body\'>" +
											 ...
											 "<h5 id=\'entryDetailsAuthors" + d.id + "\' class=\'entry-details-field\' tabindex=0></h5>" +
											 "<h6 id=\'entryDetailsVenue" + d.id + "\' class=\'entry-details-field\' tabindex=0></h6>" +
											 "<h6 id=\'entryDetailsType" + d.id + "\' class=\'entry-details-field\' tabindex=0></h6>" +
											 "<div class=\'vertical-delimiter\'></div>" +
											 "<div id=\'entryDetailsCategories" + d.id + "\' class=\'entry-details-field\'></div>" +
											 "<h6 id=\'entryDetailsDOI" + d.id + "\' class=\'entry-details-field\' tabindex=0></h6>" +
...
function onAddEntry(){
	...
	//var keys = ["id", "title", "year", "authors", "allAuthors", "reference", "url", "categories"];
	//var keys = ["id", "title", "venue", "year", "type", "categories", "url"];
	...
	if ($("#inputEntryType").val())
			entry.year = $("#inputEntryType").val();
...
function onAboutModalShown(){
	...
	$("#statsContainer").append(
			"<div>Total number of resources included: "
	...
	var processCategory = function(category, container, nestedLevel){
		if (category.children)
			$.each(category.children, function(i, d){
				...
				row.attr("title", d.description + ": " + d.value + " relevant resources");
